...
class CustomDPOTrainer(DPOTrainer):
    def __init__(
        ...
        teacher_model: Optional[Union["PreTrainedModel", torch.nn.Module]],
        ...
        **kwargs,
        ...
        # dpo hyperparams
        self.beta1 = finetuning_args.pref_beta
        self.beta2 = 0.001
        ...
        if teacher_model is not None:
            if self.is_deepspeed_enabled:
                if not (
                    getattr(teacher_model, "is_loaded_in_8bit", False) or getattr(teacher_model, "is_loaded_in_4bit", False)
                ):  # quantized models are already set on the correct device
    ...
    def compute_preference_loss(
        ...
        teacher_chosen_logps: Optional["torch.Tensor"],
        teacher_rejected_logps: Optional["torch.Tensor"],
        ...
        if not self.finetuning_args.use_ref_model:
            ...
            losses, chosen_rewards, rejected_rewards = self.dpo_loss(
                policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps,teacher_chosen_logps,teacher_rejected_logps
            )
    ...
    def compute_teacher_log_probs(
        ...
        with torch.no_grad(), ref_context:
            reference_chosen_logps, reference_rejected_logps, *_ = self.concatenated_forward(
                teacher_model, batch, is_ref_model=True
            )
    ...
    def get_batch_loss_metrics(
        ...
        teacher_chosen_logps, teacher_rejected_logps = self.compute_teacher_log_probs(model, batch)
        losses, chosen_rewards, rejected_rewards = self.compute_preference_loss(
            policy_chosen_logps,
            policy_rejected_logps,
            reference_chosen_logps,
            reference_rejected_logps,
            teacher_chosen_logps, 
            teacher_rejected_logps,
        )
